...
const EditField: React.FC<EditFieldProps> = ({label,value,isEditing,toggleKey,fieldKey,onChange,maxLength,options,displayValue,renderInput,showActionButtons = true,showEditControl = true,}) => {
  ...
  const usesCustomInput = typeof renderInput === "function";
  const computedLength = (() => {
    if (usesOptions || usesCustomInput || normalizedValue === "") {
      return 0;
    }
  ...
  const helperText = (() => {
    ...
    if (!usesCustomInput && !usesOptions && maxLength) {
      return `${computedLength}/${maxLength} caracteres`;
    }
    return undefined;
  })();
  ...
  const handleCancel = () => {
    if (fieldKey === "healthInsurance" || fieldKey === "healthPlan") {
      profileSend({ type: "CANCEL_PROFILE_EDIT", key: "healthInsurance" });
      profileSend({ type: "CANCEL_PROFILE_EDIT", key: "healthPlan" });
    } else {
